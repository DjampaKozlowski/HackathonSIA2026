...
from Extraction_excel import extract_traits
from lm_studio_client import query_lm_studio_with_text 
import pymupdf
import pymupdf4llm


def extract_pdf_to_dict(pdf_path): 
    """Extrait le PDF en dict {page_num: content}."""
...
def main(excel_path, pdf_path, output_path):
    # 1. Charger traits
    ...
    pages = extract_pdf_to_dict(pdf_path)
    print(f"Pages: {len(pages)}\n")
    
    # 3. Charger prompt
    prompt_path = Path(__file__).parent.parent / "prompts" / "v2" / "prompt_targeted_extraction.txt"
    ...
    complete = sum(1 for r in results.values() if r["trait"])
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python main.py <excel_path> <pdf_path> [output_path]")
        sys.exit(1)
