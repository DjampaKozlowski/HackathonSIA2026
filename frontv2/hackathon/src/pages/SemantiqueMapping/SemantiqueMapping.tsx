...
function Pill({
    ...
    const borderColor = isLeft ? theme.palette.primary.light : theme.palette.success.light;
    const bgColor = isLeft
        ? alpha(theme.palette.primary.light, 0.08)
        : alpha(theme.palette.success.light, 0.10);
    const textColor = isLeft ? theme.palette.primary.main : theme.palette.success.main;
...
function MappingLines({ imports, refs, mappings, threshold }: MappingLinesProps) {
    const theme = useTheme();
...
export default function SemantiqueMappingPage() {
    const theme = useTheme();
    const [threshold, setThreshold] = useAtom(SemantiqueMappingHelper.thresholdAtom());
    ...
    return (
        <Box sx={{ width: "100%" }}>
            <Box sx={{ maxWidth: 1100, mx: "auto" }}>
                <Stack spacing={1} alignItems="center" sx={{ textAlign: "center", mb: 5 }}>
